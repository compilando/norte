...
const rootFiles: FileEntry[] = [
  ...
  { name: ".github", kind: "folder", size: "—", date: "Jul 10" },
  { name: "LICENSE", kind: "markdown", size: "11 KB", date: "Jul 08" },
];
...
const srcFiles: FileEntry[] = [
  ...
  { name: "journal.rs", kind: "rust", size: "9.6 KB", date: "Jul 10" },
  { name: "provider.rs", kind: "rust", size: "7.4 KB", date: "Jul 09" },
];
...
const destinationFiles: FileEntry[] = [
  ...
  { name: "norte-macos", kind: "lock", size: "8.4 MB", date: "Jul 08" },
  { name: "norte-linux", kind: "lock", size: "7.9 MB", date: "Jul 08" },
];
...
function FileRow({ file, selected = false, copied = false }: { file: FileEntry; selected?: boolean; copied?: boolean }) {
  return (
    <motion.div
      ...
      className={`grid h-9 grid-cols-[minmax(0,1fr)_58px_62px] items-center px-2 text-[11px] sm:grid-cols-[minmax(0,1fr)_72px_78px] sm:text-xs ${selected ? "bg-phosphor/[0.12] text-ink" : "text-muted"}`}
    >
      ...
      <span className="text-right text-muted/70">{file.date}</span>
    </motion.div>
  );
}
...
function Pane({ path, files, activeIndex, copiedFile }: { path: string; files: FileEntry[]; activeIndex?: number; copiedFile?: FileEntry }) {
  return (
    <div className="min-w-0 flex-1 overflow-hidden bg-[#0e0e10]">
      <div className="flex h-11 items-center border-b border-line bg-surface px-3 text-[11px] sm:text-xs">
      ...
      <div className="grid h-8 grid-cols-[minmax(0,1fr)_58px_62px] items-center border-b border-line/70 px-2 text-[10px] uppercase tracking-wider text-muted/70 sm:grid-cols-[minmax(0,1fr)_72px_78px]">
    ...
    <div className="relative w-full" aria-label="Animated dual-pane file commander demonstration">
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-surface shadow-terminal">
        <div className="flex h-11 items-center justify-between border-b border-line bg-[#161619] px-4">
          ...
          <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted">[NOMBRE] · workspace</span>
          <span className="font-mono text-[11px] text-phosphor">NORMAL</span>
        </div>
        <div className="relative flex h-[350px] font-mono sm:h-[380px]">
        ...
        <div className="flex h-10 items-center justify-between gap-1 overflow-hidden border-t border-line bg-[#161619] px-2 font-mono">
          {shortcuts.map(([key, label]) => (
            <span key={key} className={`whitespace-nowrap text-[10px] text-muted sm:text-[11px] ${copying && key === "F5" ? "text-phosphor" : ""}`}>
      ...
      <div className="mt-3 flex items-center justify-between px-1 font-mono text-[11px] uppercase tracking-[0.11em] text-muted">
        <span className="flex min-w-0 items-center"><span className="mr-2 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-phosphor shadow-[0_0_8px_#4ADE80]" /><span className="text-phosphor">{activity[0]}</span><span className="ml-2 truncate normal-case tracking-normal text-muted">{activity[1]}</span></span>
        <span className="ml-3 shrink-0">Rust core · local session</span>
      </div>
    </div>
  );
}
