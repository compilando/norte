...
- [ ] **Step 1: the failing tests.** Four, and the third is the one that matters:
...
- [ ] **Step 2:** `just t norte-core` — watch them fail.
- [ ] **Step 3: implement.** Replace the `OnceCell` with a state machine under
...
- [ ] **Step 4:** every transition reaches the sink, RECOVERY included.
- [ ] **Step 5:** `just t norte-core`, `just c`, `security-reviewer`, commit.
...
- [ ] **Step 1: the failing test.** A `journal.db` that is not a database (or
...
- [ ] **Step 3: implement.** `Failed` refuses the mutation; `Busy` continues
...
- [ ] **Step 4:** the refusal is a Fluent string in both locales, and it says
  what to do (the file, and that removing or fixing it restores journalling).
- [ ] **Step 5:** `security-reviewer` — this is the task that changes what an
...
- [ ] **Step 1: the failing tests.** A cancelled `DeleteTree` that removed part
...
- [ ] **Step 3: implement.** `destroy_tree` returns what it removed even on
  ...
  find it.
- [ ] **Step 4:** `security-reviewer`, commit.
...
- [ ] **Step 1: the failing test.** Re-declare the format on an anchored
  journal and assert `AnchorVerdict::HashMismatch` at `seq 0`.
- [ ] **Step 2:** watch it fail.
- [ ] **Step 3: implement.** One extra MAC'd line at `seq 0`, written at
...
- [ ] **Step 4:** amend ADR 0046 in place where it concedes the hole, rather
...
- [ ] Whole-branch `security-reviewer` and a coherence pass: do tasks 1 and 2
...
- [ ] `just ci` once. Grep for `^error` and `Summary`, not a pipe's exit code.
- [ ] Close #179, #178, #160, #186, #146 with what each established.
