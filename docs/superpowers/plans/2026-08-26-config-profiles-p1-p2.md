...
- **Session body schema: there are TWO numbers, and they move together.** `norte_frontend::session::SCHEMA_VERSION` (`crates/norte-frontend/src/session.rs:24`) decides whether a *body* can be read; `norte_core::ui_session::disk::SCHEMA_VERSION` (`crates/norte-core/src/ui_session/disk.rs:32`) decides whether the *file* can be overwritten. They live in different crates because the core cannot depend on the frontend, and nothing ties them at compile time — bumping only the frontend's makes the core refuse its own file at every start, forever, behind a `warn!`. Both go **1 → 2** in Task 6. `norte-tui`'s `las_dos_versiones_de_esquema_van_del_brazo` is the only test that sees both, and it does not run under `just t norte-frontend`.
