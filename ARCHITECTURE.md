...
| `norte-gui-tauri` | The reference graphical renderer (ADR 0066/0067): a Tauri 2 window over `norte-ui-host`, plus a plain-TypeScript webview that paints and does nothing else. The ONLY crate that may name Tauri, WRY or a JavaScript runtime. Out of the default gate until its go/no-go closes (`just gui-ci` is its own); it needs WebKitGTK, GTK3 and libsoup3 from the system. | MIT OR Apache-2.0 |
