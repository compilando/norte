...
pub(super) async fn next_search(
...
async fn searching_opens_its_view_and_hits_arrive_in_batches() {
    let backend = tree_with_findings("*.txt", &["mem:///casa/notas.txt", "mem:///casa/docs"]);
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();

    by_palette(&h, &mut sub, "pane.search").await;
    ...
    let first = next_search(&mut sub).await.expect("the view opens");
    ...
    for _ in 0..20 {
        let Some(v) = next_search(&mut sub).await else {
            continue;
        };
        if !v.rows.is_empty() {
            with_rows = Some(v);
...
async fn the_skipped_notice_stays_silent_at_zero_and_is_marked_above_it() {
    for (skipped, expects_notice) in [(None, false), (Some(0), false), (Some(2), true)] {
        ...
        f.skipped = skipped;
        let (_h, snap) = host_tree(Arc::new(f)).await;
        let note = &listing_of(&snap, 1).skipped_note;
...
async fn the_header_says_names_are_reinterpreted_and_how_much_is_marked() {
    ...
    let with_names = snapshot_until(&h, &mut sub, "the header with the encoding", |s| {
        let b = listing_of(s, 1);
    ...
    let marked = snapshot_until(&h, &mut sub, "the header with what is marked", |s| {
        let b = listing_of(s, 1);
...
async fn a_failed_search_says_so_and_does_not_fake_zero_hits() {
    let mut f = Fake::default();
    f.tree.clone_from(&fake_tree().tree);
    f.search_outcome = Some(norte_proto::TaskState::Failed {
        error: norte_proto::Error::PermissionDenied,
    });
    let (h, _snap) = host_tree(Arc::new(f)).await;
    let mut sub = h.subscribe();

    let _ = search(&h, &mut sub, "*.txt").await;
    let view = snapshot_until(&h, &mut sub, "the search with its outcome", |s| {
...
async fn a_search_that_never_gets_queued_stops_saying_it_is_searching() {
    let mut f = Fake::default();
    f.tree.clone_from(&fake_tree().tree);
    f.search_error = Some(norte_proto::Error::ProviderUnavailable { retryable: false });
    let (h, _snap) = host_tree(Arc::new(f)).await;
    let mut sub = h.subscribe();

    let _ = search(&h, &mut sub, "*.txt").await;
    let view = snapshot_until(&h, &mut sub, "the search that never started", |s| {
        s.search.clone().filter(|b| !b.running)
    })
    .await;
...
async fn a_cancelled_search_does_not_read_as_finished() {
    let mut f = Fake::default();
    f.tree.clone_from(&fake_tree().tree);
    f.search_outcome = Some(norte_proto::TaskState::Cancelled);
    let (h, _snap) = host_tree(Arc::new(f)).await;
    let mut sub = h.subscribe();

    let _ = search(&h, &mut sub, "*.txt").await;
    let view = snapshot_until(&h, &mut sub, "the cancelled search", |s| {
        s.search.clone().filter(|b| !b.running)
    })
    .await;
...
pub(super) async fn search(
    ...
    by_palette(h, sub, "pane.search").await;
    h.dispatch(UiAction::Resync).await.expect("host alive");
    ...
    .await
    .expect("host alive");
    next_search(sub).await.expect("the view opens")
}
...
async fn closing_a_search_with_no_hits_cancels_it_anyway() {
    let backend = tree_with_findings("*.txt", &["mem:///casa/notas.txt"]);
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();

    let v = search(&h, &mut sub, "*.zzz").await;
    ...
    h.dispatch(UiAction::Resync).await.expect("host alive");
    assert!(
        next_snapshot(&mut sub).await.search.is_none(),
        "the view closes"
    );
    assert_eq!(
...
///
/// `key_of_an_overlay` routed nine surfaces and NOT the dialog, which is
...
async fn a_dialog_keeps_the_keyboard() {
    ...
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let before = next_snapshot(&mut sub).await;
    ...
    for k in ["Backspace", "ArrowDown", "Home"] {
        h.dispatch(press(k)).await.expect("host alive");
    }
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let during = next_snapshot(&mut sub).await;
...
async fn escape_cancels_the_topmost_dialog() {
    let backend = tree_with_findings("*.txt", &[]);
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();
...
async fn going_to_a_result_navigates_and_leaves_the_cursor_on_it() {
    ...
    for _ in 0..20 {
        h.dispatch(UiAction::Resync).await.expect("host alive");
        let snap = next_snapshot(&mut sub).await;
        if listing(&snap).path_display.contains("docs") {
            assert!(snap.search.is_none(), "the search closes on going");
            let under_cursor = listing(&snap)
                .rows
                .iter()
                .find(|r| Some(r.key) == listing(&snap).cursor)
...
async fn an_empty_pattern_launches_nothing() {
    let backend = tree_with_findings("*", &[]);
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();
    by_palette(&h, &mut sub, "pane.search").await;
    h.dispatch(UiAction::Resync).await.expect("host alive");
...
async fn the_forms_filters_reach_the_wire() {
    ...
    let requested = backend.params_search.lock().expect("mutex").clone();
...
async fn a_hostile_hit_is_marked() {
    ...
    let backend = tree_with_findings("*", &[hostile]);
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();
    by_palette(&h, &mut sub, "pane.search").await;
    h.dispatch(UiAction::Resync).await.expect("host alive");
...
async fn the_window_searches_by_meaning() {
    ...
    *fake.semantic.lock().expect("semantics") = Some(vec![
    ...
    run_by_palette(&h, &mut sub, "pane.semantic-search").await;
    ...
    let mut view = next_search(&mut sub).await.expect("the search opens");
    for _ in 0..20 {
        if !view.rows.is_empty() {
            break;
        }
        view = next_search(&mut sub).await.expect("still open");
    }
...
async fn a_semantic_search_with_no_index_says_it_needs_building() {
    let fake = tree_as_fake();
    // With no `semantic`: the fake answers `NotFound`.
    ...
    for _ in 0..40 {
        if next_notice(&mut sub).await == "msg-semantic-no-index" {
            return;
        }
    }
...
async fn an_empty_ai_instruction_returns_the_field() {
    ...
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();
    run_by_palette(&h, &mut sub, "pane.ai-rename").await;
    let id = next_dialogs(&mut sub).await[0].id;
    h.dispatch(UiAction::Dialog {
        id,
        choice: "confirm".to_owned(),
...
pub(super) async fn run_by_palette(
    h: &UiHost,
    sub: &mut norte_ui_host::controller::UiSubscription,
    command: &str,
) {
    let ack = execute_via_palette_ack(h, sub, command).await;
    assert!(
...
/// Like [`run_by_palette`], but returning the ACK: what is sometimes
/// checked is the refusal.
pub(super) async fn execute_via_palette_ack(
    h: &UiHost,
    sub: &mut norte_ui_host::controller::UiSubscription,
    command: &str,
) -> ActionAck {
