...
fn row_of(a: &norte_ui_host::dto::SettingsView, id: &str) -> u32 {
    let mut flat = 0usize;
    for s in &a.sections {
        match s {
...
async fn with_a_filter_the_cursor_points_to_the_visible_row() {
    ...
    .await
    .expect("host alive");
    let a = next_settings(&mut sub).await.expect("still open");
    assert!(
        a.shown < total,
...
async fn with_a_filter_the_dialog_asks_about_the_right_setting() {
    ...
    let mut sub = h.subscribe();
    h.dispatch(press("F11")).await.expect("host alive");
    next_settings(&mut sub).await.expect("opens");
    h.dispatch(UiAction::SettingsQuery {
    ...
    assert_eq!(row_of(&a, "ui.editor"), 0);
    h.dispatch(UiAction::SettingsSelectRow { row: 0 })
        .await
        .expect("host alive");
    next_settings(&mut sub).await.expect("still open");
    h.dispatch(press("Enter")).await.expect("host alive");

    let body = snapshot_until(
        &h,
        &mut sub,
...
async fn resetting_with_another_layer_underneath_says_so() {
    ...
    .expect("write user");
    let (h, _snap) = host_with_stacked_layers(system.path(), user.path()).await;
    let mut sub = h.subscribe();
    let a = settings_over(&h, &mut sub, "ui.theme").await;
    let row = row_of(&a, "ui.theme");
    ...
    let written = snapshot_until(&h, &mut sub, "theme removed from the user's", |_| {
    ...
    let said = snapshot_until(&h, &mut sub, "it says so", |s| {
        s.status
            .message
            .clone()
...
async fn cycling_a_setting_with_enter_writes_it_and_refreshes_the_row() {
    ...
    let (h, _snap) = host_with_layers(root.path()).await;
    let mut sub = h.subscribe();
    let a = settings_over(&h, &mut sub, "ui.show-hidden").await;
    ...
    let written = snapshot_until(&h, &mut sub, "show_hidden written", |_| {
        toml_de(root.path()).filter(|s| s.contains("show_hidden = true"))
    })
    .await;
    assert!(written.contains("[ui]"), "in its section: {written}");
    let a = snapshot_until(&h, &mut sub, "the row shows the new value", |s| {
        s.settings
            .clone()
    ...
    assert_eq!(
        a.cursor,
        u64::from(row_of(&a, "ui.show-hidden")),
...
async fn a_text_setting_asks_for_the_value_in_a_dialog_and_confirming_writes_it() {
    ...
    let (h, _snap) = host_with_layers(root.path()).await;
    let mut sub = h.subscribe();
    settings_over(&h, &mut sub, "ui.font").await;
    ...
    let written = snapshot_until(&h, &mut sub, "font written", |_| {
        toml_de(root.path()).filter(|s| s.contains("font = \"Fira Code\""))
    })
    .await;
    assert!(written.contains("[ui]"), "in its section: {written}");
    snapshot_until(&h, &mut sub, "the row shows the new value", |s| {
        s.settings
            .clone()
...
async fn an_out_of_range_integer_is_not_written_and_says_so() {
    ...
    let (h, _snap) = host_with_layers(root.path()).await;
    let mut sub = h.subscribe();
    settings_over(&h, &mut sub, "ui.font-size").await;
...
async fn with_no_user_layer_it_does_not_write_and_says_so() {
    ...
    let mut sub = h.subscribe();
    settings_over(&h, &mut sub, "ui.show-hidden").await;

    h.dispatch(press("Enter")).await.expect("host alive");
    assert_eq!(next_notice(&mut sub).await, "host-no-config-dir");
}
...
async fn cycling_the_theme_from_settings_applies_it_live() {
    ...
    let written = snapshot_until(&h, &mut sub, "theme written", |_| {
...
/// started with `--profile`, and `write_dir` only looked there.
...
async fn with_a_profile_set_the_setting_is_written_to_the_profile() {
    ...
    snapshot_until(&h, &mut sub, "the profile picker with its list", |s| {
        s.profiles
            .as_ref()
    ...
    .await;
    h.dispatch(press("Enter")).await.expect("host alive");
    snapshot_until(&h, &mut sub, "the profile set", |s| {
    ...
    let written = snapshot_until(&h, &mut sub, "show_hidden written to the profile", |_| {
    ...
    snapshot_until(&h, &mut sub, "the row shows the profile's value", |s| {
        s.settings
            .clone()
            .filter(|a| valor_de(a, "ui.show-hidden") == "true")
    })
...
async fn with_the_prompt_open_a_double_click_does_nothing() {
    ...
    let (h, _snap) = host_with_layers(root.path()).await;
    let mut sub = h.subscribe();
    let a = settings_over(&h, &mut sub, "ui.font").await;
    h.dispatch(press("Enter")).await.expect("host alive");
    let d = next_dialogs(&mut sub).await;
    assert_eq!(d.len(), 1);
    ...
    let snap = snapshot_until(&h, &mut sub, "a snapshot", |s| Some(s.dialogs.len())).await;
...
async fn a_double_click_activates_the_row_like_enter() {
    ...
    snapshot_until(&h, &mut sub, "show_hidden written by the mouse", |_| {
        toml_de(root.path()).filter(|s| s.contains("show_hidden = true"))
    })
    .await;
}
