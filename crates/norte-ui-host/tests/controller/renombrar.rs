...
async fn an_untouched_name_renames_nothing() {
    ...
    let mut sub = h.subscribe();
    let b = listado(&snap);
    let hostil = b.rows.iter().find(|r| r.hostile).expect("there is one");
...
/// host already declared it in `hechos()`.
#[tokio::test]
...
async fn a_template_batch_is_reviewed_like_the_ais() {
    ...
    let prompt = foto_hasta(&h, &mut sub, "the template prompt", |s| {
        s.dialogs
            .iter()
...
async fn a_location_that_refuses_to_write_dims_the_delete() {
    ...
    assert!(
        !fila.enabled,
        "deleting is not possible here and the page offers it turned off: {fila:?}"
    );
    assert_eq!(
        fila.reason,
        norte_i18n::t_in(norte_i18n::Lang::Es, "reason-read-only"),
        "and it says WHY, not just that it can't: {fila:?}"
    );
}
...
async fn a_relisting_does_not_relight_what_the_location_still_refuses() {
    ...
    let despues = foto_hasta(&h, &mut sub, "help after the re-listing", |s| {
        s.help.clone()
    })
    .await;
    assert!(
...
async fn a_read_only_destination_dims_the_copy_and_not_the_delete() {
    ...
    // the other one: the reader is in `/casa`, which writes, looking at
    // `/casa/docs`, which does not.
    ...
    assert!(
        !copiar.enabled,
        "the destination does not accept the copy: {copiar:?}"
    );
    assert_eq!(
        copiar.reason,
        norte_i18n::t_in(norte_i18n::Lang::Es, "reason-read-only"),
    ...
    assert!(
        ...
        "deleting happens at the SOURCE, which does write: dimming it would \
         be counting the wrong slot's impediment ({borrar:?})"
    );
}
...
pub(super) async fn pagina_de_ayuda(
    ...
    while row < u32::try_from(pagina.sidebar.len()).expect("fits") {
        ...
        h.dispatch(UiAction::HelpSelectTopic { row })
            .await
            .expect("host alive");
        pagina = siguiente_ayuda(sub).await.expect("still open");
        row += 1;
    }
    assert_eq!(pagina.topic_id, topico, "the sidebar carries the page");
    pagina
}
...
async fn entering_a_compressed_file_navigates_inside() {
    ...
    let dentro = foto_hasta(&h, &mut sub, "the listing inside the archive", |s| {
        let b = listado(s);
        b.path_display
            .contains("zip")
    ...
    assert!(
        ...
        "it enters the container, it is not handed to the desktop: {dentro}"
    );
}
...
async fn closing_the_window_asks_if_the_config_says_so() {
    ...
    let dialogo = foto_hasta(&h, &mut sub, "the quit dialog", |s| {
...
async fn the_quit_key_closes_the_window() {
    ...
    let ack = h.dispatch(tecla("F10")).await.expect("host alive");
    assert!(matches!(ack, ActionAck::Applied { .. }), "{ack:?}");
    let efecto = tokio::time::timeout(std::time::Duration::from_secs(2), efectos.recv())
        .await
        .expect("the effect comes out")
...
async fn the_quick_search_mode_comes_from_the_config() {
    ...
    let modo = foto_hasta(&h, &mut sub, "the search open", |s| {
...
async fn a_declared_opener_governs_in_the_window() {
    ...
    let norte_ui_host::dto::NativeEffect::RunProgram { argv, cwd, .. } = efecto else {
        panic!("with a declared rule THAT program is run, not the desktop's: {efecto:?}");
    };
    ...
    assert!(
        ...
        "the program is resolved to an absolute path BEFORE giving it a cwd (ADR 0082): {como_texto:?}"
    );
    assert!(
        como_texto[1].ends_with("/casa/notas.txt"),
        "and `%f` is the pointed-at file: {como_texto:?}"
    );
    assert_eq!(
...
async fn with_no_declared_rule_it_opens_with_the_desktop() {
    ...
    assert!(
        ...
        "with no rule, the desktop: {efecto:?}"
    );
}
...
async fn the_configured_editor_governs_in_the_window() {
    ...
    let norte_ui_host::dto::NativeEffect::RunProgram { argv, .. } = efecto else {
        panic!("with `[ui] editor` set, THAT editor is run: {efecto:?}");
    };
    let como_texto: Vec<String> = argv
        .iter()
        .map(|a| String::from_utf8_lossy(a).into_owned())
...
async fn comparing_two_files_runs_the_comparator_and_shows_its_output() {
    ...
    let norte_ui_host::dto::NativeEffect::RunProgram {
        ...
        panic!("expected to run a program: {efecto:?}");
    };
    ...
    assert!(
        ...
        "the program is resolved to an absolute path (ADR 0082): {como_texto:?}"
    );
    assert_eq!(
        &como_texto[1..],
    ...
    .await
    .expect("host alive");
    let con_salida = foto_hasta(&h, &mut sub, "the program's output", |s| {
    ...
    foto_hasta(&h, &mut sub, "the closed panel", |s| {
...
async fn a_plan_is_reviewed_before_being_applied() {
    ...
    let primera = siguiente_revision(&mut sub).await.expect("opens");
...
async fn approving_sends_the_batch_with_the_cores_hash() {
    ...
    let mut sub = h.subscribe();
    pedir_plan(&h, &mut sub).await;
    let mut v = siguiente_revision(&mut sub).await.expect("opens");
    for _ in 0..40 {
        if v.confirmable {
            break;
        }
    ...
    h.dispatch(tecla("y")).await.expect("host alive");
    let lotes = anotados(&backend, "the batch approved", 1, |f| {
        f.lotes.lock().expect("lotes").clone()
    })
    .await;
    assert_eq!(lotes.len(), 1, "ONE task for the whole batch");
    ...
    assert_eq!(hash, &hash_de_prueba(), "the hash is the one the core gave");
}
...
async fn a_non_applicable_plan_is_not_approved() {
    ...
    let mut sub = h.subscribe();
    pedir_plan(&h, &mut sub).await;
    let mut r = siguiente_revision(&mut sub).await.expect("opens");
    for _ in 0..40 {
        if !r.detail.is_empty() {
            break;
        }
        r = siguiente_revision(&mut sub).await.expect("still open");
    }
    ...
    h.dispatch(tecla("y")).await.expect("host alive");
    let ack = h.dispatch(tecla("y")).await.expect("host alive");
    assert_eq!(
        ack,
        ActionAck::Unavailable {
...
async fn an_invalid_pair_brings_down_the_whole_plan() {
    ...
    hasta(&backend, "the model's plan, already served", |f| {
    ...
    assert!(foto.status.message.is_some(), "and it is said");
}
...
async fn a_plan_that_arrives_late_does_not_reopen_what_was_closed() {
    ...
    h.dispatch(tecla("Escape")).await.expect("host alive");
    hasta(&backend, "the late plan, already served", |f| {
        f.en_calma().then_some(())
    })
    .await;
    asentar().await;
...
async fn discarding_closes_and_applies_nothing() {
    ...
    let mut sub = h.subscribe();
    pedir_plan(&h, &mut sub).await;
    siguiente_revision(&mut sub).await.expect("opens");

    h.dispatch(tecla("Escape")).await.expect("host alive");
    asentar().await;
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let foto = siguiente_foto(&mut sub).await;
    assert!(foto.ai_rename.is_none(), "it closed and stays closed");
    assert!(backend.lotes.lock().expect("lotes").is_empty());
}
...
async fn a_hostile_name_from_the_plan_is_marked() {
    ...
    let alterado = String::from_utf8(bytes).expect("the corpus one is UTF-8");
    let pares = [("ep1.mkv", alterado.as_str())];
    let backend = falso_con_plan(&pares, None);
    let (h, _snap) = host_arbol(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();
    pedir_plan(&h, &mut sub).await;
    let r = siguiente_revision(&mut sub).await.expect("opens");
    assert!(
...
async fn a_long_plan_is_scrolled_through() {
    ...
    let mut bajado = siguiente_revision(&mut sub).await.expect("still open");
    for _ in 0..10 {
        if bajado.first_visible > 0 {
            break;
        }
        bajado = siguiente_revision(&mut sub).await.expect("still open");
    }
    ...
    let tope = siguiente_revision(&mut sub).await.expect("still open");
    assert!(
        tope.first_visible + tope.pairs.len() as u64 <= tope.total,
        "the window does not go past the plan: {tope:?}"
    );
}
...
async fn with_a_plan_in_flight_escape_closes_the_palette() {
    ...
    .await
    .expect("host alive");
    let _ = siguiente_paleta(&mut sub).await.expect("opens");
    h.dispatch(tecla("Escape")).await.expect("host alive");
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let foto = siguiente_foto(&mut sub).await;
    assert!(foto.palette.is_none(), "Escape closed the palette");
...
async fn with_a_plan_in_flight_escape_cancels_the_filter() {
    ...
    // `siguiente_foto` consumes envelopes until it finds one, so it could
    // swallow the patch and then wait forever for an event that already
    // passed. A `Resync` re-sends the STATE.
    puerta.abrir();
    let r = foto_hasta(&h, &mut sub, "the plan's review landed", |foto| {
...
async fn discarding_a_review_does_not_kill_the_next_request() {
    ...
    let r1 = siguiente_revision(&mut sub).await.expect("the first opens");
...
async fn escape_discards_the_review_on_the_first_try() {
    ...
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let foto = siguiente_foto(&mut sub).await;
    assert!(foto.ai_rename.is_none(), "it left on the first try");
}
...
async fn a_name_that_starts_with_the_arrow_does_not_pretend_to_be_a_destination() {
    ...
    let trampa = String::from_utf8(bytes).expect("the corpus one is UTF-8");
...
async fn a_stuck_batch_says_so_and_gives_the_current_name() {
    ...
    *falso.informe.lock().expect("informe") =
    ...
    assert!(
        ...
        "it says what it is called NOW: {cuerpo}"
    );
