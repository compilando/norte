...
async fn sorting_by_column_uses_the_shared_rule() {
    ...
    let reversed = next_snapshot(&mut sub).await;
...
async fn an_attribute_sorts_from_its_header() {
    let (h, _snap) = UiHost::start(UiHostOptions {
        backend: fake_tree() as Arc<dyn norte_ui_host::HostBackend>,
        initial_dir: dir(),
    ...
    for _ in 0..20 {
        ...
        if mode(&snap).sort.as_deref() == Some("asc") {
            assert!(
                listing(&snap).columns[0].sort.is_none(),
...
pub(super) fn large_tree_with_plugins(n: usize) -> Arc<Fake> {
...
async fn a_plugin_columns_value_reaches_the_row_named_by_its_manifest() {
    let backend = large_tree_with_plugins(3);
    let (h, _snap) = UiHost::start(UiHostOptions {
        ...
        user_layouts: Vec::new(),
        profile: None,
        columns: columns_of(&["name", "plugin:acme.git/status"]),
        effects: norte_ui_host::commands::Effects::Full,
        log_ring: None,
    })
    .await
    ...
    let snap = wait_snapshot(&h, &mut sub, "the plugin's cell to arrive", |f| {
        listing(f)
            .rows
            .iter()
...
async fn plugins_are_only_asked_about_the_window() {
    ...
    let (h, _snap) = UiHost::start(UiHostOptions {
        ...
        user_layouts: Vec::new(),
        profile: None,
        columns: columns_of(&["name", "size", "plugin:acme.git/status"]),
        effects: norte_ui_host::commands::Effects::Full,
        log_ring: None,
    })
    .await
...
async fn disabling_a_decorator_from_the_manager_removes_its_badges() {
    ...
    for _ in 0..30 {
        h.dispatch(UiAction::Resync).await.expect("host alive");
        let snap = next_snapshot(&mut sub).await;
        if listing(&snap).rows.iter().any(|r| !r.badge.is_empty()) {
            with_badge = true;
            break;
        }
    }
...
async fn a_provider_with_no_count_asserts_nothing() {
    ...
    let (h, snap) = host_tree(Arc::new(f)).await;
    let _ = &h;
    assert!(listing(&snap).skipped_note.is_empty());
}
...
async fn the_skipped_count_does_not_survive_a_cd() {
    ...
    let mut sub = h.subscribe();
    assert!(
        !listing(&snap).skipped_note.is_empty(),
        "the first one does"
    );
...
pub(super) async fn selector_columns(
    h: &UiHost,
    sub: &mut norte_ui_host::UiSubscription,
) -> norte_ui_host::dto::ColumnsPickerView {
    by_palette(h, sub, "pane.columns").await;
    for _ in 0..20 {
        h.dispatch(UiAction::Resync).await.expect("host alive");
        if let Some(c) = next_snapshot(sub).await.columns.clone() {
            return c;
        }
    }
...
async fn the_columns_picker_says_its_scope_and_that_it_does_not_save() {
    let (h, _snap) = host_tree(fake_tree()).await;
    let mut sub = h.subscribe();
    let c = selector_columns(&h, &mut sub).await;
...
async fn enabling_an_attr_column_re_lists() {
    ...
    let (h, _snap) = UiHost::start(UiHostOptions {
        ...
        // fingerprint.
        columns: columns_of(&["name", "size", "attr:posix.mode"]),
        effects: norte_ui_host::commands::Effects::Full,
        log_ring: None,
    })
    .await
    ...
    let before = backend.listings.load(Ordering::SeqCst);
    ...
    for _ in 0..20 {
        h.dispatch(UiAction::Resync).await.expect("host alive");
        if next_snapshot(&mut sub).await.columns.is_none() {
            closed = true;
            break;
        }
    }
    ...
    for _ in 0..20 {
        if backend.listings.load(Ordering::SeqCst) > before {
            return;
        }
        h.dispatch(UiAction::Resync).await.expect("host alive");
        let _ = next_snapshot(&mut sub).await;
...
async fn changing_the_column_order_does_not_re_list() {
    ...
    let (h, _snap) = host_tree(Arc::clone(&backend)).await;
    let mut sub = h.subscribe();
    let _ = selector_columns(&h, &mut sub).await;
    ...
    assert_eq!(
        backend.listings.load(Ordering::SeqCst),
        before,
...
async fn the_columns_pickers_keys_are_what_its_footer_announces() {
    ...
    h.dispatch(press(" ")).await.expect("host alive");
    let still = next_columns(&h, &mut sub).await;
    assert!(
    ...
    h.dispatch(press("ArrowDown")).await.expect("host alive");
    let on_another = next_columns(&h, &mut sub).await;
    ...
    .await
    .expect("host alive");
    let moved = next_columns(&h, &mut sub).await;
    ...
    for _ in 0..moved.rows.len() + 2 {
        let v = next_columns(&h, &mut sub).await;
        ...
        if !row.format.is_empty() && !row.format_locked {
            ...
            h.dispatch(press("f")).await.expect("host alive");
            let after = next_columns(&h, &mut sub).await;
            assert_ne!(
...
pub(super) async fn next_columns(
    h: &UiHost,
    sub: &mut norte_ui_host::UiSubscription,
) -> norte_ui_host::dto::ColumnsPickerView {
    for _ in 0..20 {
...
async fn resizing_a_column_fixes_its_width_in_the_header() {
    ...
    let _ = sub.recv().await.expect("the host is still alive");
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let clamped = next_snapshot(&mut sub).await;
    ...
    assert!(
        listing(&same).columns.iter().all(|c| c.id != "attr:nadie"),
