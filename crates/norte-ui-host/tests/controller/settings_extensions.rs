...
pub(super) async fn next_settings(
...
pub(super) async fn host_with_paths(paths: norte_ui_host::settings::HostPaths) -> UiHost {
...
async fn settings_show_the_shared_registry_with_its_value() {
    ...
    h.dispatch(press("F11")).await.expect("host alive");
    let a = next_settings(&mut sub).await.expect("opens");
    ...
    // motion — is said by `out_of_scope_hot` when writing.
    let live = general
        .iter()
...
pub(super) fn place(p: std::path::PathBuf) -> norte_ui_host::settings::HostPath {
...
async fn paths_are_said_and_whats_missing_is_marked() {
    ...
    let h = host_with_paths(norte_ui_host::settings::HostPaths {
        ...
        state_dir: None,
        logs_dir: None,
        socket: Some(place(tmp.path().join("daemon.sock"))),
    })
    .await;
    let mut sub = h.subscribe();
    h.dispatch(press("F11")).await.expect("host alive");
...
async fn a_hostile_path_arrives_masked_and_marked() {
    ...
    let h = host_with_paths(norte_ui_host::settings::HostPaths {
        config_layers: vec![(norte_ui_host::settings::ConfigLayer::User, place(hostile))],
        state_dir: None,
        logs_dir: None,
        socket: None,
    })
...
async fn the_cursor_stays_in_bounds_and_enter_says_so() {
    ...
    let up = next_settings(&mut sub).await.expect("still open");
    assert_eq!(up.cursor, 0, "at the very top it does not run off the top");

    h.dispatch(press("End")).await.expect("host alive");
    let end = next_settings(&mut sub).await.expect("still open");
    let total: usize = end
        .sections
        .iter()
        .map(|s| match s {
    ...
    snapshot_until(&h, &mut sub, "esc closes them", |s| {
...
async fn with_settings_open_the_listing_does_not_move() {
    ...
    let mut sub = h.subscribe();
    h.dispatch(press("F11")).await.expect("host alive");
    let _ = next_settings(&mut sub).await.expect("opens");

    h.dispatch(press("j")).await.expect("host alive");
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let snap = next_snapshot(&mut sub).await;
    assert_eq!(listing(&snap).cursor, before, "the listing did not move");
...
pub(super) async fn next_extensions(
    sub: &mut norte_ui_host::UiSubscription,
...
async fn the_manager_shows_whats_installed_and_its_status() {
    let mut backend = tree_with_plugins(
    ...
    let first = next_extensions(&mut sub).await.expect("opens");
    assert!(
        first.loading,
...
async fn the_card_shows_the_schema_with_its_effective_value() {
    let mut backend = tree_with_plugins(vec![extension("acme.ftp", "FTP de ACME", false)], &[]);
    std::sync::Arc::get_mut(&mut backend)
        .expect("single reference")
        .schemes
        .insert(
...
async fn moving_drops_the_card() {
    ...
    let v = next_extensions(&mut sub).await.expect("still open");
...
async fn the_first_esc_closes_the_card_and_the_second_the_manager() {
    let backend = tree_with_plugins(vec![extension("acme.ftp", "FTP de ACME", false)], &[]);
    let (h, _snap) = host_tree(backend).await;
    let mut sub = h.subscribe();
    h.dispatch(press("F12")).await.expect("host alive");
    let _ = extensions_loaded(&mut sub).await;
    ...
    h.dispatch(press("Escape")).await.expect("host alive");
    let no_card = next_extensions(&mut sub).await.expect("still open");
    assert!(no_card.detail.is_none(), "the first esc closes the card");
    h.dispatch(press("Escape")).await.expect("host alive");
    assert!(
        next_extensions(&mut sub).await.is_none(),
...
async fn the_help_button_opens_that_extensions_page() {
    ...
    let mut page = None;
    for _ in 0..20 {
        let Some(v) = next_help(&mut sub).await else {
            continue;
        };
...
async fn an_extensions_text_arrives_masked() {
    ...
    let backend = tree_with_plugins(vec![hostile, bad], &[]);
    let (h, _snap) = host_tree(backend).await;
    let mut sub = h.subscribe();
    h.dispatch(press("F12")).await.expect("host alive");
    let v = extensions_loaded(&mut sub).await;
...
async fn a_plugin_keys_value_arrives_masked_and_marked() {
    ...
    for _ in 0..20 {
        h.dispatch(UiAction::Resync).await.expect("host alive");
        if let Some(d) = next_snapshot(&mut sub)
            .await
            .extensions
            .and_then(|e| e.detail)
        {
...
async fn with_the_manager_open_the_listing_does_not_move() {
    ...
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let snap = next_snapshot(&mut sub).await;
    assert_eq!(listing(&snap).cursor, before);
...
pub(super) async fn next_selector(
    sub: &mut norte_ui_host::UiSubscription,
...
pub(super) async fn host_with_theme(theme: norte_ui_host::pickers::HostTheme) -> UiHost {
    UiHost::start(UiHostOptions {
        backend: fake_tree(),
        initial_dir: dir(),
        initial_dir_requested: false,
...
async fn the_theme_shows_from_inside_and_says_what_it_does_not_paint() {
    let h = host_with_theme(norte_ui_host::pickers::HostTheme {
        name: "retro".to_owned(),
        roles: vec![
    ...
    h.dispatch(key_alt("9")).await.expect("host alive");
    let t = next_theme(&mut sub).await.expect("opens");
    ...
    h.dispatch(press("Escape")).await.expect("host alive");
    assert!(next_theme(&mut sub).await.is_none(), "esc closes it");
}
...
pub(super) fn volume(mount: &str, fs: &str, ro: bool) -> norte_proto::methods::Volume {
...
async fn choosing_a_volume_navigates_the_panel() {
    ...
    let first = next_selector(&mut sub).await.expect("opens");
    assert!(
    ...
    assert!(
        listing(&snap).path_display.contains("otro"),
...
async fn a_volume_with_no_size_says_so() {
    ...
    let mut v = volume("mem:///otro", "nfs4", true);
    v.total_bytes = None;
    v.free_bytes = None;
    f.volumes = vec![v];
    let (h, _snap) = host_tree(Arc::new(f)).await;
    ...
    for _ in 0..20 {
        let Some(view) = next_selector(&mut sub).await else {
            continue;
        };
...
async fn a_hooks_notice_reaches_the_bar_attributed() {
    ...
    *fake.notices_plugin.lock().expect("avisos_plugin") = Some(rx);
    ...
    let line = snapshot_until(&h, &mut sub, "the notice in the bar", |f| {
        f.status
            .message
            .clone()
    ...
    assert!(
        f_banners_empty(&h, &mut sub).await,
    ...
    .expect("the host is listening");
    let notice = super::log::snapshot_until_notice(&mut sub2, "msg-plugin-hooks-disabled").await;
    assert!(notice.contains("org.norte.rename-log"), "{notice}");
}

async fn f_banners_empty(
    h: &norte_ui_host::UiHost,
    sub: &mut norte_ui_host::UiSubscription,
) -> bool {
    snapshot_until(h, sub, "the banners", |f| Some(f.status.banners.is_empty())).await
}
