...
async fn f1_opens_the_contexts_help_and_its_prose_arrives_in_blocks() {
    ...
    let help = open_help(&h, &mut sub).await;
...
async fn the_keyboard_sheet_comes_from_the_effective_keymap() {
    ...
    .await
    .expect("host alive");
    let keys = next_help(&mut sub).await.expect("still open");
...
async fn a_row_this_window_does_not_run_arrives_disabled() {
    let (h, _snap) = host_tree(fake_tree()).await;
    let mut sub = h.subscribe();
    let mut help = open_help(&h, &mut sub).await;
    ...
    for row in 0..help.sidebar.len() {
        ...
        .await
        .expect("host alive");
        help = next_help(&mut sub).await.expect("still open");
    }
...
async fn activating_in_help_closes_and_runs_through_the_same_path() {
    ...
    for row in 0..40 {
        ...
        h.dispatch(UiAction::HelpSelectTopic { row })
            .await
            .expect("host alive");
        page = next_help(&mut sub).await.expect("still open");
    }
    ...
    .expect("host alive");
    assert!(
        next_help(&mut sub).await.is_none(),
    ...
    assert!(
        listing(&snap).rows.iter().any(|r| r.marked),
...
async fn following_a_link_opens_the_other_page_and_leaves_a_way_back() {
    let (h, _snap) = host_tree(fake_tree()).await;
    let mut sub = h.subscribe();
    let mut page = open_help(&h, &mut sub).await;
    ...
    .await
    .expect("host alive");
    let followed = next_help(&mut sub).await.expect("still open");
    assert_ne!(followed.topic_id, before, "the page changed");
    assert!(followed.can_back, "and there is somewhere to go back to");

    h.dispatch(press("Backspace")).await.expect("host alive");
    let back = next_help(&mut sub).await.expect("still open");
...
async fn the_bar_filters_and_escape_closes() {
    ...
    let filtering = next_help(&mut sub).await.expect("still open");
    assert!(filtering.filtering, "`/` opens the filter");

    h.dispatch(press("c")).await.expect("host alive");
    let typed = next_help(&mut sub).await.expect("still open");
    ...
    let no_filter = next_help(&mut sub).await.expect("still open");
...
async fn with_help_open_the_listing_does_not_move() {
    ...
    let mut sub = h.subscribe();
    let _ = open_help(&h, &mut sub).await;
...
pub(super) fn tree_with_plugins(
...
pub(super) async fn help_with_row(
    sub: &mut norte_ui_host::UiSubscription,
    needle: &str,
) -> norte_ui_host::dto::HelpView {
    for _ in 0..20 {
        let Some(v) = next_help(sub).await else {
            continue;
        };
...
async fn an_extension_with_a_page_is_read_from_help() {
    ...
    let mut sub = h.subscribe();
    h.dispatch(press("F1")).await.expect("host alive");
    let help = help_with_row(&mut sub, "FTP de ACME").await;
...
async fn with_the_viewer_open_help_keeps_the_keys() {
    ...
    h.dispatch(press("F1")).await.expect("host alive");
    let help = next_help(&mut sub).await.expect("help opens");
    assert_eq!(
    ...
    h.dispatch(press("Escape")).await.expect("host alive");
    assert!(next_help(&mut sub).await.is_none(), "esc closes help");
    h.dispatch(UiAction::Resync).await.expect("host alive");
    let snap = next_snapshot(&mut sub).await;
...
async fn the_body_scroll_keys_travel_as_a_request() {
    ...
    let help = next_help(&mut sub).await.expect("still open");
...
async fn no_help_string_carries_a_terminal_hazard() {
    ...
    for row in 0..rows {
        ...
        .await
        .expect("host alive");
        if let Some(v) = next_help(&mut sub).await {
            views.push(v);
        }
    }
