...
/// has already arrived.
async fn start_with_mark(attach: bool) -> norte_ui_host::ViewSnapshot {
    let mut fake = Fake::default();
    fake.put(
        "mem:///casa",
        vec![(b"a.txt".to_vec(), false), (b"b.txt".to_vec(), false)],
    );
    let mut session = crate::base::session_saved(1, 7, 1, "mem:///casa");
    ...
    *fake.session.lock().expect("session") = (session, true);
    ...
    snapshot_until(&h, &mut sub, "casa's listing with its two rows", |s| {
        (listing(s).rows.len() == 2).then(|| s.clone())
    })
    .await
}
...
async fn with_attach_the_window_returns_the_handoffs_marks() {
    // On the heap: the future carries the whole `State`, and on the stack
    // it goes over `large_futures`'s threshold (see `host_grande` in
    // `payload.rs`).
    let snap = Box::pin(start_with_mark(true)).await;
...
async fn without_attach_the_saved_marks_do_not_come_back() {
    let snap = Box::pin(start_with_mark(false)).await;
    assert!(
        listing(&snap).rows.iter().all(|r| !r.marked),
...
async fn the_handoff_writes_releases_and_then_opens_the_terminal() {
    ...
    by_palette(&h, &mut sub, "handoff").await;
    ...
    let puts = backend.placed.lock().expect("puestas").clone();
    ...
    assert_eq!(
        backend.loose.load(std::sync::atomic::Ordering::SeqCst),
        1,
...
async fn if_the_terminal_does_not_open_the_window_stays_and_says_so() {
    ...
    for _ in 0..10 {
        said = next_notice(&mut sub).await;
...
async fn if_it_could_not_be_released_nothing_opens() {
    ...
    by_palette(&h, &mut sub, "handoff").await;
    until(&backend, "the attempt to release", |f| {
