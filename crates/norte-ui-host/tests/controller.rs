...
/// Un backend de tabla: devuelve los nombres que se le den, y cuenta cuántas
/// veces se le pidió el listado.
struct Falso {
...
impl Falso {
    fn con(nombres: Vec<&'static str>) -> Arc<Self> {
        Arc::new(Self {
            nombres,
            listados: AtomicUsize::new(0),
            falla: false,
        })
    }
}
...
impl HostBackend for Falso {
    fn list(&self, dir: VPath) -> BoxFuture<'static, Result<Vec<Entry>, Error>> {
        ...
        if self.falla {
            return Box::pin(async { Err(Error::Internal { panic: false }) });
        }
        let entradas: Vec<Entry> = self
            .nombres
            .iter()
            .map(|n| Entry {
                path: dir.join(norte_proto::Segment::new(n.as_bytes().to_vec()).expect("segmento")),
                kind: EntryKind::File,
                size: Some(1),
                mtime_ms: None,
            ...
            .collect();
        Box::pin(async move { Ok(entradas) })
    }
}
...
async fn lo_no_implementado_se_dice() {
    let (h, _snap) = host(vec!["a"]).await;
    let ack = h
        .dispatch(UiAction::Parent { slot_id: 1 })
        .await
        .expect("host vivo");
