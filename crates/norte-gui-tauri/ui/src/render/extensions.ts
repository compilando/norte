...
export function paintExtensions(this: Screen, ext: ExtensionsView | null): void {
  ...
  // (puente 79): la fila `rows.length + j` es `errors[j]`.
  ...
  if (ext.errors.length > 0) {
    ...
    titulo.id = "extensions-errors-title";
    titulo.textContent = this.t("ext-errors-title");
    caja.append(titulo);
    const errores = document.createElement("ul");
    errores.className = "extensions-errors";
    // Es una LISTA de opciones, no una lista de prosa: sus filas llevan
    // `role="option"` y el cursor entra en ellas. Sin el `listbox` que las
    // contiene, `option` es ARIA inválido y un lector de pantalla no
    // anuncia nada al llegar a una fila rota — justo a quien no le queda
    // el ratón como salida.
