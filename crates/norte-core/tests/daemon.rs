...
async fn initialize_rechaza_version_incompatible() {
    ...
    let err = c
        ...
        .await
        .expect_err("0.1.0 no es N ni N-1 de 0.26.0");
    match err {
        ClientError::Rpc(rpc) => {
...
async fn frames_hostiles_y_formas_canonicas_crudas() {
    ...
    s.write_all(
        b"{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"client_info\":{\"name\":\"raw\",\"version\":\"0\"},\"protocol_version\":\"0.25.0\",\"encodings\":[\"json\"]}}\n",
    )
    .await
    .expect("write");
    let resp = read_frame(&mut s).await;
