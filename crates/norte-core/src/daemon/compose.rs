...
//! comes back as a typed [`Notice`], and whoever launches it prints it, in
...
/// same type returned by [`crate::team::equipar`], which is where the AI
/// ones come from.
pub use crate::team::Notice;
...
///
/// Notices go to `notices` AS THEY HAPPEN, not in the final `Ok`: if startup
...
///
/// # Errors
/// [`StartupError`]: the journal, the policy, the archive limits or the
/// `bind`. Everything else degrades with its [`Notice`].
pub async fn compose(o: Options, notices: &mut Vec<Notice>) -> Result<Daemon, StartupError> {
    ...
    match spool.sweep().await {
        ...
        Ok(r) if r.is_clean() => notices.push(Notice::SpoolsSweeps(r.removed)),
        Ok(r) => notices.push(Notice::SpoolsAMedias {
            removed: r.removed,
            failed: r.failed,
            dir: spool.dir().to_path_buf(),
        }),
        Err(e) => notices.push(Notice::UnsweptSpools {
            dir: spool.dir().to_path_buf(),
            error: e.to_string(),
        }),
    }
    ...
    let cfg = crate::blocking::spawn_blocking(crate::PolicyConfig::load)
        .await
        .map_err(|e| StartupError::Policy(std::io::Error::other(e)))?
    ...
    let engine = crate::team::with_index(engine, &dir, notices).await;
...
#[cfg(test)]
mod tests {
    use super::{Options, StartupError, compose};
    ...
    /// directory.
    ///
    /// It is also the only path through `compose` that can be tested
    ...
    async fn does_not_start_with_the_journal_already_held() {
        ...
        .await;
        assert!(
            matches!(r, Err(StartupError::Journal(_))),
            "expected Journal: {:?}",
            r.err()
        );
        assert!(notices.is_empty(), "nothing happened before the journal");
    }
}
