...
//!   [`BRAKE_AFTER_FAILURE`], so as not to pay `LOCK_WAIT` per mutation.
...
//! starts with the file busy and runs longer than [`BRAKE_AFTER_FAILURE`] started
...
pub const BRAKE_AFTER_FAILURE: std::time::Duration = std::time::Duration::from_secs(30);
...
impl LazyJournal {
    ...
    pub fn in_state_dir(state_dir: &Path) -> Self {
        Self::with_retry_brake(state_dir, BRAKE_AFTER_FAILURE)
    }

    /// Like [`Self::in_state_dir`], with another retry brake.
    ///
    /// Exists for the tests —which cannot wait for [`BRAKE_AFTER_FAILURE`] to see
    ...
    /// "unrecorded session" warning, and [`BRAKE_AFTER_FAILURE`] worth of real
