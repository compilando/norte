...
// `suspend_tests` los llama por `super::`, así que tienen que estar en la raíz
// del binario y no dentro del módulo. La producción no los nombra —se fueron
// con `suspend.rs`—, y ese módulo de test se queda hasta que salgan
// `submit_command_line` y `shell_remote_message`, que aún viven aquí.
#[cfg(test)]
use norte_tui::suspend::{suspension_outcome, write_resume_prologue};
