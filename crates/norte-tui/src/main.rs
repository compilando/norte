...
use norte_tui::app::{
    ALLOW_EXTENSIONS, ALLOW_PLUGIN_CONFIG, App, CompareState, ExtensionManager, HelpOutcome,
    HelpView, Modal, NavPopupKind, PAGE, Palette, SearchState, Settings, Trail, TrailStep,
    TransferKind, config_error_category, detail_for_bar, error_category, error_message,
    keymaps_error_category,
};
use norte_tui::config::{self, Layers, WatchMode};
use norte_tui::fill::{Fill, apply_fill_msg, release_refreshed_fill};
...
// Con los selectores y el overlay de ajustes fuera, la producción del binario
// ya no nombra `presets` —quien lo llamaba (`on_settings_key`) se fue—, pero
// `help_key_tests` lo fabrica por `use super::*`. Explícito bajo `cfg(test)`
// porque el build de PRODUCCIÓN no ve ese uso y `cargo fix` lo borraría.
#[cfg(test)]
use norte_tui::keymap::presets;
// Con el editor de atajos fuera, el binario ya no nombra estos cuatro en
// producción: quien los usaba (`build_keymaps`, `known_commands`,
// `shortcut_rows`) se fue con él. Sus `mod` de test todavía sí.
use norte_tui::jobs::{
...
#[cfg(test)]
use norte_tui::keymap::{COMMANDS, DIALOG_COMMANDS, Effective, Screen};
...
use norte_tui::nav;
use norte_tui::navigate::{Cd, apply_cd, cache_capabilities, cd, cd_in, cd_landed_pane, listing};
...
// `modal_help_toggle` y `refuses_over_modal` ya no se llaman en la producción
// del binario —sus llamantes se fueron a `mutations.rs` y a `overlays.rs`—,
// pero `help_key_tests` todavía los afirma.
#[cfg(test)]
use norte_tui::overlays::{modal_help_toggle, refuses_over_modal};
...
use norte_tui::screens::{
    apply_theme, on_columns_key, on_connections_picker_key, on_layout_picker_key, on_nav_popup_key,
    on_places_key, on_processes_key, on_settings_key, on_theme_picker_key, on_tree_key,
    open_drive_popup, pane_attr_ids, refresh_places_drives, refresh_places_favorites,
};
