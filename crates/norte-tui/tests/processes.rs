...
fn pulsar(app: &mut App, resolver: &mut Resolver, seq: Vec<Chord>) -> Option<String> {
    ...
    for chord in seq {
        match resolver.push(chord) {
            ...
            otro => panic!("the key does not resolve to a command: {otro:?}"),
        }
    }
    last
}
...
fn el_preset_que_ate_layout_processes_lo_ata_en_las_dos_pantallas() {
    ...
    for nombre in presets::NAMES {
        ...
        assert_eq!(
            ata(Screen::Browse),
            ata(Screen::Dialog),
            "{nombre} binds layout.processes on one screen and not the other"
        );
    }
}
...
fn la_fila_dice_la_clase_y_sobre_que_actua_sin_el_id() {
    ...
    assert!(pantalla.contains("copy"), "the class: {pantalla}");
    assert!(pantalla.contains("foto.jpg"), "the operand: {pantalla}");
    assert!(
        !pantalla.contains("7318349021"),
        "the task id is not painted: {pantalla}"
    );
}
...
fn la_franja_nombra_el_operando_con_el_panel_cerrado() {
    ...
    let pantalla = pintar(&app, 80, 24);
    assert!(pantalla.contains("delete"), "the class: {pantalla}");
    assert!(pantalla.contains("borrame"), "the operand: {pantalla}");
}
...
fn una_ruta_larga_no_desborda_la_fila() {
    ...
    for linea in pantalla.lines() {
        ...
        assert!(
            fila.chars().count() <= 60,
            "a row overran the width: {fila:?}"
        );
    }
    assert!(pantalla.contains('…'), "clipped: {pantalla}");
    // The task's row: the name's tail, the bar AND the status, in that
    // order and within the width.
    let fila = pantalla
        .lines()
        .find(|l| l.contains("final.txt"))
        .unwrap_or_else(|| panic!("no row names the operand: {pantalla}"));
    assert!(
        fila.contains("25%"),
        "the status was not eaten by the edge: {fila:?}"
    );
    assert!(
        pantalla.contains("final.txt"),
        "the tail survives: {pantalla}"
    );
}
