...
setting-ui-alt-menu-desc = Whether pressing and releasing Alt on its own opens the menu bar in the TUI. Needs a terminal with the kitty keyboard protocol (kitty, foot, WezTerm, Ghostty); tmux, xterm and GNOME Terminal cannot report a lone Alt. Off by default: in that mode letters typed with a dead key (é, ñ via compose) may arrive without their accent. The GUI always has it.
