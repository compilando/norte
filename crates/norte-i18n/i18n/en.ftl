...
modal-ai-rename-plan = Rename plan — review it
...
msg-viewer-no-sibling = no more files of this kind in this listing
...
msg-profile-start-orphans = [profile.start] names { $n } slot(s) this layout does not place: { $ids }
...
cli-warn-archive-invalid = warning: norte.toml could not be read ({ $error }); the [archive] limits stay at their defaults
...
cli-plugin-uninstall-daemon-stale = a daemon is running and will keep listing it until it restarts; `norte --daemon plugin uninstall` removes it through the daemon
...
failed-reason-secret-empty = the configured secret is EMPTY
failed-reason-secret-not-utf8 = the secret is not valid text
failed-reason-secret-store = the secret store could not be read
...
col-attr-posix-owner = Owner
col-attr-posix-group = Group
...
settings-hint = [tab] side · [[/]] section · [enter] edit · [ctrl+r] reset · [esc] close
...
ext-broken-only-uninstall = this extension did not load: the only thing to do with it here is uninstall it
ext-broken-not-id = this directory cannot be uninstalled from here: its name is not an extension id, or a loaded extension already uses that id — remove it by hand
...
picker-history-title-left = History — left panel
picker-history-title-right = History — right panel
picker-popular-title = Popular directories
...
history-title-left = History — left panel
history-title-right = History — right panel
popular-title = Popular directories
popular-empty = no popular directories yet
history-mark-current = here
history-mark-forward = forward
...
menu-item-pane-test-archive = Test the archive
menu-item-pane-split-file = Split the file
menu-item-pane-combine-files = Combine the pieces
menu-item-pane-checksum = Checksums
menu-item-pane-checksum-verify = Verify checksums
menu-item-mark-restore = Restore marks
menu-item-mark-extension-add = Mark by extension
...
menu-item-nav-set-jump-point = Set jump point
menu-item-pane-history-left = Left panel history
menu-item-pane-history-right = Right panel history
menu-item-pane-popular = Popular directories
menu-item-nav-forward = Forward
menu-item-pane-history = History
menu-item-pane-hotlist = Bookmarks
menu-item-pane-select-drive = Volumes
...
menu-item-profile-save-as = Save profile as…
...
help-cmd-nav-jump-back = back to this panel's jump point
help-cmd-nav-set-jump-point = mark this directory as the panel's jump point
help-cmd-pane-history-left = directory history of the left panel
help-cmd-pane-history-right = directory history of the right panel
...
# And this one is different: it could not be ASKED. Calling a relieved daemon
# tampering is a false accusation, and teaching people to ignore the message
# above is what makes it useless the day it is true.
msg-edit-created-unchecked = the file was created, but what is there could not be checked: the editor did not open
...
help-cmd-viewer-zoom-in = zoom in on the image
help-cmd-viewer-zoom-out = zoom out of the image
help-cmd-viewer-zoom-fit = fit the image to the viewer
help-cmd-viewer-next = go on to the next file of the same kind
help-cmd-viewer-prev = go back to the previous file of the same kind
help-cmd-viewer-left = one column left
help-cmd-viewer-right = one column right
...
search-exclude = skip folders:
search-min-size = at least:
search-max-size = at most:
search-days = changed in last (days):
search-encoding = read text as:
search-regex = [F2] regex: { $on }
search-case = [F3] case: { $on }
search-whole-word = [F4] whole word: { $on }
search-recursive = [F5] subfolders: { $on }
search-kinds = [F6] looking for: { $what }
search-kinds-any = anything
...
status-session-detached = session not saved
...
cli-ai-rename-invalid = the model's plan is not valid (a name is not a legal file name); nothing was renamed
cli-ai-rename-collisions = the plan cannot be applied as it stands; nothing was renamed:
cli-ai-rename-stale = the directory changed while you read the plan; nothing was renamed, run it again
...
dialog-cmd-confirm-other = other panel
dialog-cmd-clear = clear
...
setting-ui-lang-desc = Interface language. Leave unset to negotiate from the environment. For a single run, `--lang` and NORTE_LANG win over it.
...
setting-ui-alt-menu-name = Alt opens the menu
setting-ui-alt-menu-desc = Whether pressing and releasing Alt on its own opens the menu bar in the TUI. Needs a terminal with the kitty keyboard protocol (kitty, foot, WezTerm, Ghostty); tmux, xterm and GNOME Terminal cannot report a lone Alt. Off by default: in that mode the terminal sends keys, not text, so a letter typed with a dead key (é) or a symbol typed with AltGr (@, #) arrives as its base key. The GUI always has it.
...
setting-ui-panel-bar-style-desc = How the panel bar names its buttons: the panel's name with its access letter underlined (names), or the letter alone (letters). Names fall back to letters on their own when the row is narrower than sixty cells. The terminal's column (position left) draws icons: Unicode symbols with names or icons, Nerd Font glyphs with nerd, and the letter with letters.
...
setting-ui-row-stripes-desc = Paint the odd rows of a listing on a band of their own, so a wide row is easy to follow from its name to its date. The colour is the theme's; the cursor, a marked row and the pointer are painted over the band.
...
setting-ui-theme-light-name = Theme when the desktop is light
setting-ui-theme-light-desc = The theme the window paints when the desktop prefers a light colour scheme: a preset name or a path to a theme file, like `theme`. Empty paints `theme` in both schemes. The terminal ignores it.
setting-ui-theme-dark-name = Theme when the desktop is dark
setting-ui-theme-dark-desc = The theme the window paints when the desktop prefers a dark colour scheme. Empty paints `theme` in both schemes. The terminal ignores it.
splash-title = norte
splash-daemon-embedded = core in this process
splash-daemon-connected = connected to the daemon
splash-daemon-connecting = connecting…
splash-recent = Where you were
splash-popular = Where you go most
splash-bookmarks = Bookmarks
splash-profiles = Profiles
splash-hint = any key continues
splash-hint-home = a number opens its row · any other key continues
setting-ui-splash-name = Start screen
...
setting-ui-history-size-desc = How many directories each panel remembers: the ones the history list shows and the steps "back" walks. Between 5 and 64; 64 is what the session keeps.
...
host-created-unchecked = the file was created, but what is there could not be checked: it was not opened
