...
at a time, and returns to the list after the last one; `→` does the same and
`←` walks the ring backwards. Inside an extension's settings, `Tab`, `←` or
`Esc` go back to the buttons. The button that holds
